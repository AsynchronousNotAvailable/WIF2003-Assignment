...
function AddProduct() {
  ...
  const [productDesc, setProductDesc] = useState(product ? product.description : "");
  const [productPrice, setProductPrice] = useState(product ? product.pricePerUnit: "");
  ...
  async function handleSubmit(event)  {
    ...
    //   Status: Math.floor(Math.random() * 4),
      ...
      category: category,
      deleted: false,
      image: images && images.length > 0 ? await extractBase64Strings(images) : [],
    };
    /* var fs = require('fs');
              ...
              <div className="flex w-full inline-block">
                ...
                >
                  <span className="flex text-[#7450DF] pl-3">
                    <img className="mt-2 h-3" src={cross_icon}></img>
                ...
                >
                  <span className="text-white flex ml-4">
                    <img className="h-4 mt-1" src={add_icon}></img>
                    <p className="ml-1">{isAdd ? "Add Product" : "Save"}</p>
                  </span>
                </button>
              </div>
