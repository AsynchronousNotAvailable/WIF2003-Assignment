...
const App = () => {
    ...
    useEffect(() => {
        const fetchMonthlyPurchaseAmount = async () => {
            try {
                const res = await axios.get(
                    `http://localhost:1234/api/customers/analysis/${customerId}/monthlypurchase`
                );
        ...
        const fetchOrderStatusCategory = async () => {
            try {
                const res = await axios.get(
                    `http://localhost:1234/api/customers/analysis/${customerId}/orderStatusCategory`
                );
        ...
        const fetchPurchaseCategory = async () => {
            try {
                const res = await axios.get(
                    `http://localhost:1234/api/customers/analysis/${customerId}/purchaseCategory`
                );
        ...
        const fetchPurchaseHistory = async () => {
            try {
                const res = await axios.get(
                    `http://localhost:1234/api/customers/analysis/${customerId}/purchaseHistory`
                );
    ...
    const orderStatusSummary = Object.entries(orderStatusCategory).map(
        ([key, value]) => {
    ...
    const productCategorySummary = Object.entries(purchaseCategory).map(
        ([key, value]) => {
            return { label: key, value: value };
        }
    );
                ...
                <section className="flex-1  flex flex-col w-full gap-10 h-full">
                    <section className="flex flex-row w-full gap-5 h-[280px]">
                        <section className="flex-1 shadow-[0_3px_10px_rgb(0,0,0,0.2)]  p-5 rounded-xl flex flex-row">
                            <section className="flex items-center w-1/3 px-5">
                                <p className="text-xl font-bold font-sans tracking-normal">
                    ...
                    <section className="flex flex-row w-full gap-10">
                        <section className=" flex flex-col font-sans flex-1 shadow-[0_3px_10px_rgb(0,0,0,0.2)] p-5  h-[300px] rounded-xl">
                            ...
                            <p className="font-sans text-[#616262]">
                                Shows your order status summary
                            </p>
                        ...
                        <section className="flex flex-col font-sans flex-1  shadow-[0_3px_10px_rgb(0,0,0,0.2)] p-5 h-[300px] rounded-xl">
                            <span className="font-sans font-bold">
                                Most Frequently Bought Product Categories
                            </span>
                            <p className="font-sans text-[#616262]">
                                Shows your most frequently bought product
                                categories
                            </p>
                            <PieChart
                                series={[
                    ...
                    <section className="flex flex-row  justify-between h-[200px] gap-10 items-center  ">
                        <section
                            className="flex flex-col w-1/6  shadow-[0_3px_10px_rgb(0,0,0,0.2)]  rounded-xl h-full justify-center items-center"
                        ...
                        >
                            ...
                            />
                            <p className="font-sans font-semibold mt-[20px]">
                                Order Status
                            </p>{" "}
                        </section>
                        ...
                        <section
                            className="flex flex-col flex-1 shadow-[0_3px_10px_rgb(0,0,0,0.2)]  rounded-xl h-full justify-center items-center"
                            onClick={() =>
                                navigate("/customer/purchase_history")
                            }
                        >
                            <img
                                src="/SalesIcon.png"
                                style={{ width: "50px" }}
                            />
                            <p className="font-semibold font-sans mt-[20px]">
                                Purchase History
                            </p>
                        </section>
                    </section>

                    <section className="flex flex-row justify-evenly"></section>
                </section>
            </section>
        </div>
    );
